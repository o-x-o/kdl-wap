 ...
 * @option_param {function} clipStart 开始裁剪的回调函数。this指向图片对象，并将图片地址作为参数传入
...
function photoClip(container, option) {
	...
	var clipWidth = parseUnit(option.width),
		clipHeight = parseUnit(option.height),
	...
	function adjustImg(src, kbs){
		var quality = (compress && kbs!=null && kbs > 1024)?(1024 / kbs):1;
		// 图片大于1M，判断是否需要压缩
	...
	function loadInputFile(){
		...
		if (!/image\/\w+/.test(thisfile.type)) {
			...
			sourceExt=thisfile.type;
			var fileReader = new FileReader();
			fileReader.onprogress = function(e) {
				/*console.log((e.loaded / e.total * 100).toFixed() + "%");*/
			};
			fileReader.onload = function(e) {
				var flag=loadStart.call(fileReader, thisfile);
				if(flag==false){
					return;
				}
				$(view).attr('title',thisfile.name);//保存文件名
				adjustImg(this.result, e.total/1024);
			};
			fileReader.onerror = function(e) {
				loadError.call(this, e);
			};
			fileReader.readAsDataURL(thisfile); // 读取文件内容
		}
	}
	if(source){
		//createImg(source);
		var ext=source.match(/\.(jpg|jpeg|gif|png|bmp)$/mg);
		if(ext!=null)sourceExt=ext;
		adjustImg(source);
	}else{
		loadInputFile.call($file[0]);
	}
	function clearInput(){this.value = "";}
	$file.attr("accept", "image/jpg,image/jpeg,image/png,image/gif,image/tiff,image/bmp")
		.unbind("change")
		.unbind("click")
	...
	function resetScroll() {
		...
		var viewWidth=getViewSize()[0],
			viewHeight=getViewSize()[1];
		
		var posX = (viewWidth - imgWidth * myScroll.options.zoomStart) * .5,
			posY = (viewHeight - imgHeight * myScroll.options.zoomStart) * .5;
	...
	function initEvent() {
		...
		$("#clipWrap .rotateBtn").click(function(){
			rotateCW();
		});
	}
	...
	function rotateBy(angle, point) {
		...
		if (!point) {
			var viewWidth=getViewSize()[0],
				viewHeight=getViewSize()[1];
			loacl = loaclToLoacl($moveLayer, $clipView, viewWidth * .5, viewHeight * .5);
		} else {
	...
	function clipImg() {
		...
		clipStart.call($img[0], $img[0]&&$img[0].src);
		$('.lazy_cover,.lazy_tip').show();
		var local = loaclToLoacl($moveLayer, $clipView);
		var scale = myScroll.scale;
		var ctx = canvas.getContext("2d");
		var winScale = getViewSize()[2];
		ctx.clearRect(0, 0, canvas.width, canvas.height);
		ctx.save();

		if (strictSize) {
			ctx.scale(scale / winScale, scale / winScale);
		} else {
			canvas.width = clipWidth / scale * winScale;
			canvas.height = clipHeight / scale * winScale;
		}
		...
		$view.css("background-image", "url("+ dataURL +")").attr("src",dataURL);
	...
	function calculateScale(width, height) {
		var viewWidth=getViewSize()[0],
			viewHeight=getViewSize()[1];
		myScroll.options.zoomMin = getScale(viewWidth, viewHeight, width, height);
	...
	function compressImg(sourceImgObj, quality, angleOffset, outputFormat){
		...
		// IOS 设备上 canvas 宽或高的乘积如果大于 4096*4096，就有可能导致应用崩溃闪退
		// 因此这里需要缩放
		if (isIOS() && drawWidth*drawHeight > 4096*4096 ) {
			var minSide = Math.min(drawWidth, drawHeight);
			var maxSide = Math.max(drawWidth, drawHeight);
			minSide = minSide / maxSide * 4096;
			maxSide = 4096;
	...
	function init() {
		...
		var $mask_left = $("<div class='photo-clip-mask-left'>").css({
			...
			"width": "auto",
			"height": viewHeight,
			"margin-right": viewWidth/2,
			"margin-top": -viewHeight/2,
			"margin-bottom": -viewHeight/2,
		...
		var $clip_area = $("<div class='photo-clip-area'>").css({
			...
			"width": viewWidth,
			"height": viewHeight,
			"margin-left": -viewWidth/2 - 1,
			"margin-top": -viewHeight/2 - 1
		}).appendTo($mask);
	...
	/*为适应屏幕大小返回适应屏幕的剪裁框尺寸以及缩放率*/
	function getViewSize(){
		var winWidth = $(window).width(),
			winHeight = $(window).height(),
			w_ratio = winWidth/winHeight,
			c_ratio = clipWidth/clipHeight,
			scale = 1,
			viewWidth,
			viewHeight;
		if(c_ratio > w_ratio && clipWidth > winWidth){
			viewWidth = winWidth;
			scale = winWidth/clipWidth;
			viewHeight = clipHeight*scale;
		}else if(c_ratio < w_ratio && clipHeight > winHeight){
			viewHeight = winHeight;
			scale = winHeight/clipHeight;
			viewWidth = clipWidth*scale;
		}else{
			viewWidth = clipWidth;
			viewHeight = clipHeight;
		}
		return [viewWidth,viewHeight,scale];
	}
}
